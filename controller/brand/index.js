...
const mod = new (require('../../model/brand'))(app.pool);
const { validateBrand } = require('./validation');
...
router.post("/", auth, validateBrand, async (req, res)=>{
    let data = req.body;
    if(data.file){
        util.writeBase64(data.file, {createThumb:false, path:'./private/images/brand/', ext:'.jpg'}, async function (resu) {
            if (resu.status > 0) {
                return res.status(422).json({status: 1, ...resu});
            } else {
                data.brand_path = 'brand/'+resu.data.file_name;
...
router.put("/:id", auth, validateBrand, async (req, res) => {
    const data = req.body;
    let param = {
