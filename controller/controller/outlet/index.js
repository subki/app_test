...
const mod = new (require('../../model/outlet'))(app.pool);
const { validateOutlet } = require('./validation');
...
router.post("/", auth, validateOutlet, async (req, res)=>{
    let data = req.body;
    if(data.file){
        util.writeBase64(data.file, {createThumb:false, path:'./private/images/outlet/', ext:'.jpg'}, async function (resu) {
            if (resu.status > 0) {
                return res.status(422).json({status: 1, ...resu});
            } else {
                data.path_image = 'outlet/'+resu.data.file_name;
...
router.put("/:id", auth, validateOutlet, async (req, res) => {
    const data = req.body;
    let param = {
